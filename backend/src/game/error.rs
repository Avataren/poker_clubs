...
impl fmt::Display for GameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GameError::NotEnoughChips { required, available } => {
                write!(f, "Not enough chips. Required: {}, Available: {}", required, available)
            }
            ...
            GameError::RaiseTooSmall { min_raise, attempted } => {
                write!(f, "Raise amount {} is too small. Minimum raise: {}", attempted, min_raise)
            }
            ...
            GameError::InvalidPhase { expected, actual } => {
                write!(f, "Invalid phase. Expected: {}, Actual: {}", expected, actual)
            }
            GameError::GameInProgress => write!(f, "Cannot perform action while game is in progress"),
...
mod tests {
    ...
    fn test_error_display() {
        let err = GameError::RaiseTooSmall { min_raise: 100, attempted: 50 };
        assert_eq!(err.to_string(), "Raise amount 50 is too small. Minimum raise: 100");
